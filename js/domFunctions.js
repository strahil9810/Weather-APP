export const setPlaceholderText = () => {
    ...
    window.innerWidth < 400 ? (input.placeholder = "City, State, Country") : (input.placeholder = "Име на Град, Държава");
}
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    const feels = createElem("div", "feels", `Усеща се като: ${Math.round(Number(weatherObj.current.feels_like))}°`);
    const maxTemp = createElem("div", "maxtemp", `Макс: ${Math.round(Number(weatherObj.daily[0].temp.max))}°`);
    const minTemp = createElem("div", "mintemp", `Мин: ${Math.round(Number(weatherObj.daily[0].temp.min))}°`);
    const humidity = createElem("div", "humidity", `Влажност: ${weatherObj.current.humidity}%`);
    const wind = createElem("div", "wind", `Вятър: ${Math.round(Number(weatherObj.current.wind_speed))} ${windUnit}`);
...
const getDayAbbreviation = (data) => {
    ...
    const utcString = dateObj.toLocaleString('bg', {  weekday: 'long' });
...
const createDailyForecastIcon = (icon, altText) => {
    ...
    if(window.innerWidth < 768 || window.innerHeight < 1025) {
        ...
        img.sec = `https://openweathermap.org/img/wn/${icon}@2x.png`;
    }
    img.alt = altText;
    return img;
}
